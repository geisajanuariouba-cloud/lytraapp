...
export const getDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // If no tasks exist for today, insert FALLBACK_TASKS synchronously with real UUIDs.
    // This is fast (no AI call) and guarantees the frontend always gets persisted rows
    // with real UUIDs it can toggle, complete, and query in History.
    // AI-personalised tasks are generated in the background and will replace fallback
    // tasks on the NEXT day's first load.
    if (tasks.length === 0) {
      const habit = onb.data?.habit ?? "foco e disciplina";
      const triggers: string[] = onb.data?.triggers ?? [];
      console.log("[daily_mission] no tasks — inserting fallback tasks now, habit=", habit);

      // Insert FALLBACK_TASKS with real DB rows (UUID generated by the DB)
      ...
      if (insertErr) {
        ...
        // Still try to return whatever is there
        ...
        // Fire AI generation in the background for TOMORROW's tasks
        // (comment: we skip this to keep the response fast; AI runs on next new day)
      }
    }
